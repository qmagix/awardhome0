...
- **Keys** (`utils/partnerAuth.js`, issued at `/admin/partner-keys`, superadmin): one long-lived bearer key per partner, SHA-256 hash stored only, shown once at issuance, issued only after a signed data agreement (template at `docs/partner_data_agreement.md`; the form requires an agreement reference). Missing/bogus/revoked keys all get byte-identical 401s.
- **Pricing (agreement §9):** prepaid blocks of **$10 per 100 dancer records** ($0.10/record). A credit = one unique dancer record retrieved per calendar month (retries/re-checks of the same dancer that month are free); disambiguation searches are free. The admin keys page shows each key's billable count for the month, computed from the audit log. Credit exhaustion is enforced administratively for the pilot (daily quota + revoke are the levers).
- **Audit + quotas:** every lookup lands append-only in `partner_query_log` — which key, what was asked, which dancer ids came back. It is the abuse signal, the quota ledger (daily quota is counted from the log), and the answer to "who has looked up my child?" — including retroactively after a safety suppression. Three throttle layers: per-IP 120/min **before** auth (a keyless flood or buggy retry loop never reaches the key-lookup DB read; `PARTNER_IP_RATE_LIMIT` overrides), then two **per-partner** limits read from the key's own row — burst rate/min (default 60) and daily quota (default 200), both superadmin-editable inline at `/admin/partner-keys` because partners differ in trust and usage shape; changes apply on the key's next request, no restart or reissue. The quota counts repeat lookups too (only *billing* dedupes re-checks), so a runaway client with a valid key self-arrests in minutes.
...
- **Safety suppression (superadmin, `/admin/suppressions`):** on a family's safety request (protective orders, families fleeing someone), a superadmin can suppress a dancer so they read as **nonexistent on every public surface** — profile page (404, identical to a missing dancer), search and typeahead, homepage leaderboards, studio pages and cast lists, the embeddable widget, card embeds, the claim page, guest mobile API reads, and the partner API. Award rows stay (group awards keep their other dancers; studio totals don't shift) — only the dancer's name and profile stop resolving. The owning household keeps its signed-in views (`/my-dancers`, manage pages, the mobile app), because suppression protects the family and must not lock them out of their own record. Suppression survives every dancer-merge path (admin merge, studio roster merges, the weekly auto-merge). Distinct from the owner-facing hide-from-search/rankings toggles: those are preferences a family sets themselves; this is a protective action reachable only by superadmins. Implemented in `utils/suppression.js` (`notSuppressedSql()` fragment, same idiom as `excludeIndependentSql`).
