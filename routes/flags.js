...
// reject, revoke, and flags all behave alike). The reinstate guard is
// GROUP-scoped to match: a human reinstate on ANY copy of the content
// blocks auto-dark for every copy (else flagging a sibling id would
// bypass the human's decision).
const TYPES = {
  ack: {
    ...
    wasReinstated: (db, id, row) => db.get(`
      SELECT 1 FROM content_flags cf
      JOIN award_acknowledgements aa ON aa.id = cf.content_id
      WHERE cf.content_type = 'ack' AND cf.status = 'resolved_reinstated'
        AND aa.dancer_id = ? AND aa.message = ? LIMIT 1`,
      [row.dancer_id, row.message]),
  },
  award_photo: {
    ...
    wasReinstated: (db, id, row) => db.get(`
      SELECT 1 FROM content_flags cf
      JOIN award_card_photos ap ON ap.id = cf.content_id
      WHERE cf.content_type = 'award_photo' AND cf.status = 'resolved_reinstated'
        AND ap.dancer_id = ? AND ap.photo_url = ? LIMIT 1`,
      [row.dancer_id, row.photo_url]),
  },
  default_photo: {
    ...
    wasReinstated: (db, id) => db.get(
      "SELECT 1 FROM content_flags WHERE content_type = 'default_photo' AND content_id = ? AND status = 'resolved_reinstated' LIMIT 1", [id]),
  },
};
...
router.post('/api/flag-card-content', flagLimiter, async (req, res) => {
  ...
  // Auto-dark only if no copy of this content was ever human-reinstated
  // (group-scoped griefing guard).
  if (content.status === 'approved') {
    const reinstated = await TYPES[type].wasReinstated(db, id, content);
