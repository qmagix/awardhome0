...
- [ ] Encore PDF data parsing/import: 71 results PDFs queued in tobeprocessed/pdf/oncore (2022-2026, all verified "Encore Performing Arts Showcase, Inc.", DanceBug S3 source) — build the PDF extractor + idempotent importer (dedupe by event/routine/place/category per ETL rules), bridge the oncore-dir vs encore-org-slug mismatch, run identically on local + prod. Award vocab reference: docs/org_top_awards.md Encore section (bands High Gold→"Can I Get an Encore!" are adjudications, NOT awards).
