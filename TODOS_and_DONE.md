...
### Features
- [ ] Feature-flag release infrastructure (designed 2026-08-13): feature_flags table (off/beta/on + scheduled flip date), flagOn() helper cached via utils/cache, superadmin /admin/features page, cron-executed scheduled flips, per-feature beta cohort, "What's New" changelog + release email on each flip. First flags: thank_you_notes, award_photos (deploy dark, release on cadence). Principle: separate deploy from release, never hold code in branches.
- [ ] Auto-moderation pipeline for card content (designed 2026-08-13): tier 1 rules (URLs/emails/phones/handles/profanity — also PII protection for minors), tier 2 OpenAI Moderation API (free) clean→auto-approve, tier 3 trusted repeat authors skip checks; propagated copies inherit verdicts; "auto-approved recently" feed with one-click revoke on /admin/card-content; moderation_mode setting (manual/assisted/auto). Notes first; photos keep human review for first-time uploaders (vision moderation as pre-filter only).
