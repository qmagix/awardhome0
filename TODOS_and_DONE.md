...
- [~] Encore PDF import (2026-08-25: TOOLING DONE, awaiting review): scripts/extract_encore_pdfs.js parsed all 71 PDFs → 28,266 rows in tobeprocessed/pdf/oncore/txt/ (gitignored) for HUMAN REVIEW — 5,790 flagged rows (LIG? = ligature guess tt/ti, SECTION? = combined-subdivision tables, WRAP? = merged wrapped lines; see _extraction_summary.txt). After review: node scripts/import_encore_txt.js --apply (dry-run default; verified idempotent on a throwaway DB: 71 events, 28,266 awards, 115 new studios, re-run inserts 0). 3,278 rows from 2022-era PDFs have no studio column → import with studio_id NULL (future backfill possible). Prod parity: txt is gitignored — rsync the REVIEWED txt dir to prod, then run the same import there.
