// Step 1 of the Refresh Dance Competition import (two-step, like Encore/NexStar):
// scrape refreshdance.com results into reviewable txt files under
// tobeprocessed/refresh/txt/ — NOTHING touches the database. Review, then
// run import_refresh_txt.js (step 2).
//
// Refresh is KAR-family (started 2026) (same DanceBug platform as Rainbow): results index
...
//
// Pages cache under raw/refresh/<year>/ via fetch_cache (scrape_log
// bookkeeping), so re-runs are cheap and deterministic.
//
// Usage (repo root):  node scripts/scrape_refresh_to_txt.js [years...]
//                     default years: 2026 (the tour started in 2026)
const fs = require('fs');
const path = require('path');
const cheerio = require('cheerio');
const { fetchWithCache } = require('./fetch_cache');

const txtDir = path.join(__dirname, '..', 'tobeprocessed', 'refresh', 'txt');
...
async function scrapeEvent(url, year) {
  const { data } = await fetchWithCache(url, 'refresh', year);
  const $ = cheerio.load(data);

  // "Purchase, NY - 3/6/2026 Results — Refresh Dance Competition"
  const title = $('title').text() || '';
  let city = '', dateStr = '';
  const tm = title.match(/^(.*?)\s*-\s*([\d/]+)\s*Results/);
  if (tm) { city = tm[1].trim(); dateStr = tm[2].trim(); }
  // Refresh titles: "Baton Rouge, LA February 28, 2026 — Refresh Dance"
  if (!city) {
    const rm = title.match(/^(.*?[A-Z]{2})\s+([A-Z][a-z]+ \d{1,2}(?:-\d{1,2})?, \d{4})/);
    if (rm) { city = rm[1].trim(); dateStr = rm[2].trim(); }
  }
  let venue = '', dateRange = '';
...
async function main() {
  ...
  const runYears = years.length ? years : [2026];
  const summary = [];

  for (const year of runYears) {
    const listUrl = `https://refreshdance.com/competition/results/${year}`;
    console.log(`\n=== ${year}: ${listUrl}`);
    let listData;
    try {
      listData = (await fetchWithCache(listUrl, 'refresh', year, 'event_list')).data;
    } catch (e) {
    ...
    for (const id of ids) {
      const url = `https://refreshdance.com/competition/results/${year}/${id}`;
      try {
        const r = await scrapeEvent(url, year);
        const base = `${year}-${id}`;
        const eventName = `Refresh - ${r.city || base}${r.dateRange ? ` (${r.dateRange})` : r.dateStr ? ` (${r.dateStr})` : ''}`;
        const out = [];
        out.push('# Refresh Dance Competition extraction — REVIEW before importing (scripts/import_refresh_txt.js)');
  ...
  console.log(`Review the txt files in ${txtDir}, then run: node scripts/import_refresh_txt.js`);
}
