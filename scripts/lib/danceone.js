// Shared scraper core for DanceOne convention brands (JUMP, NUVO,
// 24SEVEN...) — they all publish on the same platform:
//   <base>/past-seasons/   every season's events in one page (the
//     ?season= query is ignored server-side; each season sits in its own
//     .year-YYYY_YYYY div, hidden by JS), giving id + city + date.
//   <base>/results/?id=N   one event, sectioned by <h3> (SPECIAL AWARDS /
//     HIGH SCORE BY AGE / HIGH SCORE BY PERFORMANCE / SCHOLARSHIPS) and
//     categorised by <h4> (e.g. "Junior : Duo/Trio", "NUbie : Solo").
//
// Table shapes, told apart by their <th> row:
//   Place | Routine / Dancer | Studio       solo placements — dancer in parens
//   Place | Routine | Studio                group placements
//   Place | Studio | Dancer                 scholarships (WINNER/RUNNER-UP)
//   Place | Studio | Dancer | Faculty       class scholarships (faculty
//                                           column is the presenting teacher —
//                                           not part of the award, ignored)
//   Studio | Routine                        Best-in-Studio style awards
//
// Pages are cached under raw/<org>/<year>/<id>.html so re-runs are
// offline and deterministic. Each brand's scrape_<org>_to_txt.js is a
// thin config around run() below.
...
async function loadIndex(cfg) {
  const html = await fetchCached(`${cfg.base}/past-seasons/`, path.join(cfg.rawDir, 'past-seasons.html'));
...
function parseEvent(html) {
  ...
  $('table.jt-results-table').each((_, table) => {
    ...
    $t.find('tbody tr').each((__, tr) => {
      ...
      } else if (shape === 'Place|Studio|Dancer|Faculty') {
        // class scholarships: 4th column names the presenting faculty
        // member, which is not part of the dancer's award — dropped
        place = cells[0]; studio = cells[1] || ''; dancers = cells[2] || '';
...
async function run(cfg) {
  ...
  const RESULTS_URL = `${cfg.base}/results/?id=`;
  fs.mkdirSync(OUT_DIR, { recursive: true });
  const events = await loadIndex(cfg);
  ...
  for (const ev of events) {
    ...
    let html;
    try {
      html = await fetchCached(RESULTS_URL + ev.id, path.join(cfg.rawDir, String(ev.year), `${ev.id}.html`));
    } catch (err) {
    ...
    const eventName = `${cfg.label} ${ev.year} ${ev.city}`;
    const out = [
      `# ${cfg.label} ${ev.season} season — scraped ${new Date().toISOString().slice(0, 10)} from ${RESULTS_URL}${ev.id}`,
