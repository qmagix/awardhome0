...
**Installing an Expo module with native code means the app must be REBUILT.**
`expo install expo-image-picker` writes the JavaScript; the native side only
reaches the device through `npm run ios`. Until then the running binary throws

```
Cannot find native module 'ExponentImagePicker'
```

and — this is the part worth knowing — a top-level `import` of that module
takes down **every module that imports it, transitively**. Adding
`expo-network` to `src/outbox/index.ts` produced: the root layout crashing on
`startOutboxSync of undefined`, two routes reported as "missing the required
default export", and "Too many screens defined" for routes that had simply
failed to load. One missing native module, four unrelated-looking symptoms.

So the rule here: **an optional native capability is required lazily, at the
point of use, and its absence is handled.**

| Module | Loaded | If missing |
|---|---|---|
| `expo-network` | lazily, per check | connectivity unknown → try the request anyway |
| `expo-image-picker` | lazily, on tap | "photos need a newer build"; the award still saves |
| `expo-sqlite` | lazily, on first queue use | the outbox fails, the app does not |
| `expo-secure-store` | at import | no session persistence (Keychain is core to auth) |

The root layout also guards `startOutboxSync`, because anything that throws
there is a blank app rather than a degraded one.
