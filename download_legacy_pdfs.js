...
const COMPETITIONS = [
  { name: 'Starpower', slug: 'starpower', ifid: 161 },
  { name: 'Revolution', slug: 'revolution', ifid: 154 },
  { name: 'Believe Talent', slug: 'believe', ifid: 152 },
  { name: 'Imagine Dance', slug: 'imagine', ifid: 150 },
  { name: 'Dream Maker', slug: 'dreammaker', ifid: 146 }
];
  ...
  try {
    ...
    $('a').each((i, el) => {
      ...
      if (href && href.toLowerCase().endsWith('.pdf') && (href.toLowerCase().includes('results') || linkText.includes('results'))) {
        ...
        const locationDateStr = (locStr ? `${locStr} - ${dateStr}` : `Unknown Location - ${year}`).trim();
...
async function run() {
  console.log('Starting Legacy PDF Results Downloader...');
  ...
  console.log(`\nFinished all downloads. Files saved to /tobeprocessed/pdf/`);
}
